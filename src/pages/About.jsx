...
const About = () => {
  const { bgPrimary, textOnPrimary, bgSecondary, bgCard, textPrimary, textSecondary, accentColor, accentColorDark } = useThemeColors()

  return (
    <Box>
      <Box bg={bgPrimary} color={textOnPrimary} py={16} px={4} transition="all 0.3s">
      ...
      <Box py={16} px={4} bg={bgSecondary} transition="all 0.3s">
        <SimpleGrid columns={{ base: 1, md: 2 }} gap={12} maxW="1200px" mx="auto" alignItems="center">
          ...
          <VStack align="start" gap={4}>
            <Text fontSize="3xl" fontWeight="bold" color={textPrimary}>Who We Are</Text>
      ...
      <Box py={16} px={4} bg={bgCard} transition="all 0.3s">
        <VStack maxW="1200px" mx="auto" gap={10}>
          <Text fontSize="3xl" fontWeight="bold" textAlign="center" color={textPrimary}>Our Core Values</Text>
          <SimpleGrid columns={{ base: 1, md: 3 }} gap={8} w="100%">
            {values.map((value, index) => (
              <VStack key={index} bg={`${bgCard === 'white' ? 'gray.50' : 'gray.700'}`} p={8} borderRadius="lg" shadow="md" gap={4}>
                <Box p={4} bg={accentColor} borderRadius="full">
                  <value.icon size={32} color={textPrimary} />
                </Box>
                <Text fontSize="xl" fontWeight="bold" color={textPrimary}>{value.title}</Text>
                <Text color={textSecondary} textAlign="center">{value.description}</Text>
              </VStack>
            ))}
          </SimpleGrid>
        </VStack>
      ...
      <Box py={16} px={4} bg={bgSecondary} transition="all 0.3s">
        <SimpleGrid columns={{ base: 2, md: 4 }} gap={8} maxW="1000px" mx="auto" textAlign="center">
          <VStack>
            <Text fontSize="4xl" fontWeight="bold" color={accentColorDark}>{accentColorDark.includes('orange') ? '50K+' : '50K+'}</Text>
            <Text color={textSecondary}>Active Students</Text>
          </VStack>
          <VStack>
            <Text fontSize="4xl" fontWeight="bold" color={accentColorDark}>200+</Text>
            <Text color={textSecondary}>Courses Available</Text>
          </VStack>
          <VStack>
            <Text fontSize="4xl" fontWeight="bold" color={accentColorDark}>100+</Text>
            <Text color={textSecondary}>Expert Instructors</Text>
          </VStack>
          <VStack>
            <Text fontSize="4xl" fontWeight="bold" color={accentColorDark}>4.8</Text>
            <Text color={textSecondary}>Average Rating</Text>
          </VStack>
        </SimpleGrid>
      </Box>
    </Box>
